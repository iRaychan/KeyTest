...
async function injectUniverse(response){
  const type=response.headers.get('content-type')||'';
  if(type&&!type.includes('text/html'))return response;
  let html=await response.text();
  if(!html.includes('universe.js'))html=html.replace(/<\/body>/i,'<script src="universe.js?v=370"></script></body>');
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
    try{
      ...
      let output=network;
      if(isKeySuiteShell(event.request,url,network))output=await injectUniverse(network.clone());
      ...
      if(isKeySuiteShell(event.request,url,cached))return injectUniverse(cached.clone());
      return cached;
    }
  })());
});
