(() => {
  ...
  let selectedSeries='',frameReady=false,queued=null,currentCurveModel='',currentCurveFamily='CHC',esFrameReady=false,esQueued=null,productDutyDirty=false,productDutyTimer=null;
  ...
  function productFlowToM3h(v,u){v=Number(v);return u==='usgpm'?v*.227124707:u==='igpm'?v*.2727654:u==='lps'?v*3.6:u==='lpm'?v*.06:v}
  function productHeadToM(v,u){v=Number(v);return u==='bar'?v*10.19716213:u==='kpa'?v*.1019716213:u==='psi'?v*.703249615:u==='ft'?v*.3048:v}
  function currentProductFrame(){return currentCurveFamily==='ES'?ensureEsFrame():ensureFrame()}
  function setProductDutyStatus(text,error=false){const el=$('productCurveDutyStatus');if(!el)return;el.textContent=text||'';el.classList.toggle('error',!!error)}
  function resetProductDutyBar(){productDutyDirty=false;clearTimeout(productDutyTimer);if($('productCurveFlow'))$('productCurveFlow').value='';if($('productCurveHead'))$('productCurveHead').value='';if($('productCurveFlowUnit'))$('productCurveFlowUnit').value='m3h';if($('productCurveHeadUnit'))$('productCurveHeadUnit').value='m';setProductDutyStatus("Enter Flow + Head to update this model\'s duty point.",false)}
  function sendProductDutyNow(){const f=$('productCurveFlow'),h=$('productCurveHead');if(!f||!h||!currentCurveModel)return false;const q=productFlowToM3h(f.value,$('productCurveFlowUnit')?.value||'m3h'),hm=productHeadToM(h.value,$('productCurveHeadUnit')?.value||'m');if(!(q>0&&hm>0)){setProductDutyStatus('Enter both Flow and Head to update the curve.',false);return false}const frame=currentProductFrame();if(!frame?.contentWindow)return false;setProductDutyStatus('Updating duty point…',false);postToFrame(frame,{type:'KEYSUITE_PRODUCT_DUTY',model:currentCurveModel,flowM3h:q,headM:hm});return true}
  function scheduleProductDuty(){productDutyDirty=true;clearTimeout(productDutyTimer);productDutyTimer=setTimeout(sendProductDutyNow,320)}
  function sendProductCurveAction(action){if(!currentCurveModel)return;const frame=currentProductFrame();if(!frame?.contentWindow)return;const dutySent=sendProductDutyNow();setTimeout(()=>postToFrame(frame,{type:'KEYSUITE_PRODUCT_ACTION',action}),dutySent?100:0)}
  ...
  function renderModels(){
    ...
    grid.querySelectorAll('[data-product-view]').forEach(button=>button.onclick=()=>{const model=button.dataset.productView;currentCurveFamily='CHC';currentCurveModel=model;resetProductDutyBar();$('productCurveTitle').textContent=model;const frame=ensureFrame(),host=$('productCurveHost');if(frame.parentNode!==host)host.appendChild(frame);frame.style.display='block';$('productCurveDialog').showModal();send(model,'view')});
  ...
  function renderEs(){
    ...
    body.querySelectorAll('[data-es-curve]').forEach(b=>b.onclick=()=>{const p=productFor(b.dataset.esCurve);if(!p)return;currentCurveFamily='ES';currentCurveModel=p.model;resetProductDutyBar();$('productCurveTitle').textContent=p.model;const frame=ensureEsFrame(),host=$('productCurveHost');if(frame.parentNode!==host)host.appendChild(frame);frame.style.display='block';$('productCurveDialog').showModal();sendEsProduct(p.model,'view')});
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    bindStaticDefaults();
    $('closeProductCurve')?.addEventListener('click',()=>$('productCurveDialog')?.close());$('exportProductCurve')?.addEventListener('click',()=>sendProductCurveAction('export'));$('productCurveAssembly')?.addEventListener('click',()=>sendProductCurveAction('assembly'));$('productCurveQuote')?.addEventListener('click',()=>sendProductCurveAction('quotation'));['productCurveFlow','productCurveHead'].forEach(id=>{$(id)?.addEventListener('input',scheduleProductDuty)});['productCurveFlowUnit','productCurveHeadUnit'].forEach(id=>{$(id)?.addEventListener('change',scheduleProductDuty)});['esProductMaterial','esProductSealType','esProductSealMaterial','esProductElastomer'].forEach(id=>{const el=$(id);if(el){bindDefaultState(el);el.addEventListener('change',()=>{if(id==='esProductSealType')esProductSealSync();if(currentCurveFamily==='ES'&&currentCurveModel)sendEsProduct(currentCurveModel,'view')})}});esProductSealSync();$('esProductSearch')?.addEventListener('input',renderEs);$('gwsProductSeries')?.addEventListener('change',()=>{if($('gwsProductModel'))$('gwsProductModel').value='ALL';renderGws()});$('gwsProductModel')?.addEventListener('change',renderGws);$('keyplcProductSearch')?.addEventListener('input',renderKeyplc);
  });
