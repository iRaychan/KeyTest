(() => {
  ...
  function renderModels(){
    ...
    $('productModelGrid').innerHTML=rows.length?rows.map(p=>`<div class="product-model-row"><h3>${esc(p.model)}</h3><div class="product-model-actions"><button class="btn secondary product-action-button icon-only" type="button" data-product-view="${esc(p.model)}" title="View Curve" aria-label="View Curve">${curveIcon()}</button><button class="btn green product-action-button" type="button" data-product-export="${esc(p.model)}">PDF</button><button class="btn secondary product-action-button" type="button" data-product-assembly="${esc(p.model)}">Add to Assembly</button><button class="btn product-action-button" type="button" data-product-add="${esc(p.model)}">Quote</button></div></div>`).join(''):'<div class="product-empty">No matching CHC models.</div>';
