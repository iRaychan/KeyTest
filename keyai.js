(() => {
  ...
  async function loadInbox(){
    ...
    }catch(error){console.error(error);box.innerHTML=`<div class="notice">KeyAI Inbox is unavailable. V3.8.3 continues to use the existing V340 database functions. ${esc(error.message||error)}</div>`}
  }
  ...
  async function load(){
    ...
    try{
      ...
      await loadInbox();
    }catch(error){console.error(error);notice(`KeyAI settings are unavailable. V3.8.3 continues to use the V340 KeyAI database functions. ${error.message||error}`);status('Settings unavailable','error')}
  }
  async function save(){
