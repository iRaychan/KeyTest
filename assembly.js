...
function applyCouplingConfiguration(d,item,values,context){
 ...
 const previous={...(item.couplingData||{})};
 item.couplingData={...previous,selectionMode:mode,autoSelected:true,managedByPumpset:true,_assemblySyncKey:'',_assemblyErrorKey:''};
 try{
  const result=window.KeySuiteCoupling?.configureAssemblyItem?.(item,{...values,selectionMode:mode},context);
  ...
  if(result?.error){
   ...
   item.couplingData={...previous,...(item.couplingData||{}),selectionMode:mode,resolvedType:explicitType,type:explicitType,managedByPumpset:true,validationReasons:result.reasons||[result.error],_assemblyErrorKey:signature,_assemblySyncKey:''};
   item.description=window.KeySuiteCoupling?.assemblyDescription?.(mode)||item.description;d.coupling_error=result.error;return result
  }
  ...
  item.couplingData={...(item.couplingData||{}),selectionMode:mode,resolvedType:resolved,type:resolved,managedByPumpset:true,_assemblySyncKey:couplingSyncSignature(d,item,context),_assemblyErrorKey:''};
  d.coupling_error=(result?.config?.validationReasons||[]).join('; ');return result||{item}
 }catch(error){
  ...
  item.couplingData={...previous,...(item.couplingData||{}),selectionMode:mode,resolvedType:explicitType,type:explicitType,managedByPumpset:true,validationReasons:[message],_assemblyErrorKey:couplingSyncSignature(d,item,context),_assemblySyncKey:''};
  item.description=window.KeySuiteCoupling?.assemblyDescription?.(mode)||item.description;d.coupling_error=`Coupling selection could not be completed: ${message}`;console.error('Coupling configuration failed',error);return {error:d.coupling_error,reasons:[message],item}
 }
}
...
function couplingOptions(item){
 ...
 const data=item.couplingData,mode=String(current?.coupling_mode||data.selectionMode||'flexible'),resolvedType=String(data.resolvedType||data.type||(/^FCL/i.test(String(data.model||''))?'pin_bush':'tyre'))==='tyre'?'tyre':'pin_bush',explicitType=mode==='tyre'?'tyre':mode==='pin_bush'?'pin_bush':resolvedType,context=window.KeySuiteCoupling?.contextFromItems?.(current?.items||[])||{};
...
function setCouplingMode(mode,rowId=''){
 ...
 withCouplingMutation(()=>{if(operation!==couplingOperationSeq)return;current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};current.coupling_mode=next;const context=window.KeySuiteCoupling?.contextFromItems?.(current.items||[])||{};let item=(rowId?current.items.find(entry=>entry.id===rowId):null)||(current.items||[]).find(entry=>entry.section==='coupling')||null;
  if(!item){item=createCouplingPlaceholder(current,context);current.items.push(item)}
  const previousResolved=item.couplingData?.resolvedType||item.couplingData?.type||'pin_bush';item.couplingData={...(item.couplingData||{}),selectionMode:next,resolvedType:next==='tyre'?'tyre':next==='pin_bush'?'pin_bush':previousResolved,type:next==='tyre'?'tyre':next==='pin_bush'?'pin_bush':previousResolved,autoSelected:true,managedByPumpset:true,manualModel:false,_assemblySyncKey:'',_assemblyErrorKey:''};
