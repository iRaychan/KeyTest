...
function applyCouplingConfiguration(d,item,values,context){
 const mode=String(values?.selectionMode||d?.coupling_mode||item?.couplingData?.selectionMode||'flexible');d.coupling_mode=mode;
 item.couplingData={...(item.couplingData||{}),selectionMode:mode,type:mode==='tyre'?'tyre':mode==='pin_bush'?'pin_bush':item.couplingData?.type||'pin_bush',autoSelected:true,managedByPumpset:true,_assemblySyncKey:'',_assemblyErrorKey:''};
 try{
  const result=window.KeySuiteCoupling?.configureAssemblyItem?.(item,values,context);
  const signature=couplingSyncSignature(d,item,context);
  if(result?.error){item.couplingData={...(item.couplingData||{}),selectionMode:mode,type:mode==='tyre'?'tyre':item.couplingData?.type||'pin_bush',managedByPumpset:true,validationReasons:result.reasons||[result.error],_assemblyErrorKey:signature,_assemblySyncKey:''};item.description=window.KeySuiteCoupling?.assemblyDescription?.(mode)||item.description;d.coupling_error=result.error;return result}
  item.couplingData={...(item.couplingData||{}),selectionMode:mode,managedByPumpset:true,_assemblySyncKey:couplingSyncSignature(d,item,context),_assemblyErrorKey:''};d.coupling_error=(result?.config?.validationReasons||[]).join('; ');return result||{item}
 }catch(error){
  const message=error?.message||String(error);item.couplingData={...(item.couplingData||{}),selectionMode:mode,type:mode==='tyre'?'tyre':item.couplingData?.type||'pin_bush',managedByPumpset:true,validationReasons:[message],_assemblyErrorKey:couplingSyncSignature(d,item,context),_assemblySyncKey:''};item.description=window.KeySuiteCoupling?.assemblyDescription?.(mode)||item.description;d.coupling_error=`Coupling selection could not be completed: ${message}`;console.error('Coupling configuration failed',error);return {error:d.coupling_error,reasons:[message],item}
 }
}
function syncCouplingItems(d=current){
 ...
 const context=window.KeySuiteCoupling.contextFromItems?.(d.items||[])||{},hasPump=Number(context.pumpCount)>0,hasMotor=Number(context.motorCount)>0,couplings=(d.items||[]).filter(item=>item.section==='coupling');
 if(d.auto_suppressed?.coupling){couplings.forEach(item=>{d.description=removeDescriptionBlock(d.description,item.description)});d.items=(d.items||[]).filter(item=>item.section!=='coupling');d.coupling_error='Coupling was removed. Use Restore Coupling to size it again.';return}
 if(!hasPump||!hasMotor){couplings.forEach(item=>{d.description=removeDescriptionBlock(d.description,item.description)});d.items=(d.items||[]).filter(item=>item.section!=='coupling');d.coupling_error=hasPump||hasMotor?'Select both Pump and Motor to size the coupling':'';return}
 d.coupling_mode=couplingMode(d);let item=couplings[0]||null;
 if(!item){item=createCouplingPlaceholder(d,context);d.items.push(item)}
 if(!item.couplingData)item.couplingData={selectionMode:d.coupling_mode,type:d.coupling_mode==='tyre'?'tyre':'pin_bush',autoSelected:true,managedByPumpset:true};
...
function couplingOptions(item){
 ...
 const data=item.couplingData,mode=String(current?.coupling_mode||data.selectionMode||'flexible'),type=mode==='tyre'?'tyre':mode==='pin_bush'?'pin_bush':String(data.type||data.resolvedType||'pin_bush')==='tyre'?'tyre':'pin_bush',context=window.KeySuiteCoupling?.contextFromItems?.(current?.items||[])||{},modelDisabled=mode==='flexible'?' disabled':'';
...
function setCouplingMode(mode,rowId=''){
 if(!current||couplingMutationDepth>0)return;const operation=++couplingOperationSeq;
 withCouplingMutation(()=>{read({skipAutomatic:true,skipCouplingDom:true});if(operation!==couplingOperationSeq)return;const next=String(mode||'flexible');current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};current.coupling_mode=next;const context=window.KeySuiteCoupling?.contextFromItems?.(current.items||[])||{};let item=(rowId?current.items.find(entry=>entry.id===rowId):null)||(current.items||[]).find(entry=>entry.section==='coupling')||null;
  if(!item){item=createCouplingPlaceholder(current,context);current.items.push(item)}
  item.couplingData={...(item.couplingData||{}),selectionMode:next,type:next==='tyre'?'tyre':next==='pin_bush'?'pin_bush':item.couplingData?.type||'pin_bush',autoSelected:true,managedByPumpset:true,manualModel:false,_assemblySyncKey:'',_assemblyErrorKey:''};applyCouplingConfiguration(current,item,{selectionMode:next},context);finishCouplingMutation()
 })
}
function updateCouplingItemFromRow(row,changedClass=''){
 if(!current||couplingMutationDepth>0||!row)return;const operation=++couplingOperationSeq,rowId=row.dataset.assemblyItem,selectedMode=row.querySelector('.assembly-coupling-type')?.value||current.coupling_mode||'flexible',selectedModel=row.querySelector('.assembly-coupling-model')?.value||'',selectedArrangement=row.querySelector('.assembly-coupling-arrangement')?.value||'';
 withCouplingMutation(()=>{read({skipAutomatic:true,skipCouplingDom:true});if(operation!==couplingOperationSeq)return;const item=current?.items?.find(entry=>entry.id===rowId);if(!item?.couplingData)return;const context=window.KeySuiteCoupling?.contextFromItems?.(current.items||[])||{},values={selectionMode:selectedMode};current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};current.coupling_mode=selectedMode;if(changedClass.includes('model'))values.model=selectedModel;if(changedClass.includes('arrangement')){values.model=selectedModel;values.arrangement=selectedArrangement}applyCouplingConfiguration(current,item,values,context);finishCouplingMutation()
 })
}
function deleteCouplingItem(rowId){if(!current)return;couplingOperationSeq+=1;withCouplingMutation(()=>{const item=current.items.find(entry=>entry.id===rowId);if(item)current.description=removeDescriptionBlock(current.description,item.description);current.auto_suppressed={...(current.auto_suppressed||{}),coupling:true};current.coupling_error='Coupling was removed. Use Restore Coupling to size it again.';current.items=current.items.filter(entry=>entry.section!=='coupling');current.description_manual=false;rebuildDescription(current);if($('assemblyDescription'))$('assemblyDescription').value=current.description||'';renderDescriptionPreview();localSave();queueCouplingRender();scheduleAutoSave(180)})}
...
function renderItems(){
 ...
 box.querySelectorAll('.assembly-delete').forEach(b=>b.onclick=()=>{const row=b.closest('[data-assembly-item]'),rowId=row?.dataset.assemblyItem||'',item=current.items.find(x=>x.id===rowId);if(item?.section==='coupling'){deleteCouplingItem(rowId);return}read({skipAutomatic:true,skipCouplingDom:true});if(item){current.description_manual=false;current.description=removeDescriptionBlock(current.description,item.description);if(['pump','motor'].includes(item.section))current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};if(autoComponent(item,'manifold'))current.auto_suppressed={...(current.auto_suppressed||{}),manifold:true};if(autoComponent(item,'tank'))current.auto_suppressed={...(current.auto_suppressed||{}),tank:true}}current.items=current.items.filter(x=>x.id!==rowId);syncAutomaticComponents(current);rebuildDescription(current);if($('assemblyDescription'))$('assemblyDescription').value=current.description||'';renderDescriptionPreview();localSave();renderItems();renderQuoteFields();scheduleAutoSave(100)});
