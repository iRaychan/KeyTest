...
let type='system',drafts=[],current=null,loaded=false,autoSaveTimer=null,saveQueue=Promise.resolve(),currentPinned=false,couplingMutationDepth=0;
...
function normalizeCapacityUnit(value){const raw=String(value||'m³/hr').trim();if(/^m(?:³|3)\/(?:h|hr)$/i.test(raw))return 'm³/hr';return CAPACITY_UNITS.find(unit=>unit.toLowerCase()===raw.toLowerCase())||'m³/hr'}
...
function parseCapacityDescription(value){const line=String(value||'').replace(/\r\n?/g,'\n').split('\n').find(row=>/^Capacity:/i.test(row.trim()));if(!line)return null;const match=line.trim().match(/^Capacity:\s*([0-9]+(?:\.[0-9]+)?)\s*(L\/s|m³\/(?:h|hr)|m3\/(?:h|hr)|L\/min|GPM)\s*@\s*([0-9]+(?:\.[0-9]+)?)\s*(Mtr|ft|bar)\s*$/i);return match?{display_capacity:true,capacity_value:match[1],capacity_unit:normalizeCapacityUnit(match[2]),head_value:match[3],head_unit:normalizeHeadUnit(match[4])}:null}
...
function createCouplingPlaceholder(d,context){const mode=couplingMode(d);return normalizeItem({id:uid(),section:'coupling',model:'',bomDescription:'No suitable Coupling model',description:window.KeySuiteCoupling?.assemblyDescription?.(mode)||'',qty:Math.max(1,Number(context.couplingQty||1)),unitPrice:0,pricingSource:{product_family:'COUPLING',pricing_mode:'assembly'},couplingData:{selectionMode:mode,type:mode==='tyre'?'tyre':'pin_bush',autoSelected:true,manualModel:false,validationReasons:['No suitable Coupling model is available.']}},'pumpset')}
function syncCouplingItems(d=current){
 ...
 if(d.auto_suppressed?.coupling){automatic.forEach(item=>{d.description=removeDescriptionBlock(d.description,item.description)});d.items=(d.items||[]).filter(item=>!automatic.includes(item));d.coupling_error='Coupling was removed. Use Restore Coupling to size it again.';return}
 ...
 if(!item){const built=window.KeySuiteCoupling.buildAssemblyItem?.(d.coupling_mode,context);item=built?normalizeItem({...built,id:uid(),section:'coupling'},'pumpset'):createCouplingPlaceholder(d,context);d.items.push(item)}
 ...
 if(result?.error){item.couplingData={...(item.couplingData||{}),selectionMode:d.coupling_mode,type:d.coupling_mode==='tyre'?'tyre':item.couplingData?.type||'pin_bush',autoSelected:true,validationReasons:result.reasons||[result.error],_assemblyErrorKey:signature,_assemblySyncKey:''};item.description=window.KeySuiteCoupling?.assemblyDescription?.(d.coupling_mode)||item.description;d.coupling_error=result.error;return}
 item.couplingData={...(item.couplingData||{}),_assemblySyncKey:couplingSyncSignature(d,item,context),_assemblyErrorKey:''};d.coupling_error=(result?.config?.validationReasons||[]).join('; ');if(previous&&previous!==item.description)d.description=replaceDescriptionBlock(d.description,previous,item.description);
...
function couplingOptions(item){
 ...
 const data=item.couplingData,mode=String(data.selectionMode||current?.coupling_mode||'flexible'),type=mode==='tyre'?'tyre':mode==='pin_bush'?'pin_bush':String(data.type||data.resolvedType||'pin_bush')==='tyre'?'tyre':'pin_bush',context=window.KeySuiteCoupling?.contextFromItems?.(current?.items||[])||{},modelDisabled=mode==='flexible'?' disabled':'';
 ...
 const warnings=(data.validationReasons||[]).filter(Boolean);if(!warnings.length&&current?.coupling_error)warnings.push(current.coupling_error);const warning=warnings.length?`<div class="assembly-coupling-warning">${esc(warnings.join('; '))}</div>`:'';
 return `<div class="assembly-motor-options assembly-coupling-options"><div class="assembly-item-option"><label>Selected Type</label><select class="assembly-coupling-type"><option value="flexible" ${mode==='flexible'?'selected':''}>Flexible</option><option value="pin_bush" ${mode==='pin_bush'?'selected':''}>Pin &amp; Bush</option><option value="tyre" ${mode==='tyre'?'selected':''}>Tyre</option></select></div><div class="assembly-item-option"><label>Model</label><select class="assembly-coupling-model"${modelDisabled}>${couplingModelOptions(type,data.model)}</select></div>${type==='tyre'?`<div class="assembly-item-option"><label>F/H Arrangement</label><select class="assembly-coupling-arrangement"${modelDisabled}>${couplingArrangementOptions(data,context)}</select></div>`:''}</div>${bushSummary}${warning}`;
}
function couplingModeControls(){
 if(type!=='pumpset')return '';const context=window.KeySuiteCoupling?.contextFromItems?.(current?.items||[])||{};if(!(Number(context.pumpCount)>0&&Number(context.motorCount)>0))return `<div class="assembly-coupling-mode-controls"><div class="muted">Select both Pump and Motor to size the coupling.</div></div>`;if(current?.auto_suppressed?.coupling)return `<div class="assembly-coupling-mode-controls"><div class="muted">Coupling removed from this Pumpset.</div><button class="btn assembly-coupling-restore" type="button">Restore Coupling</button></div>`;return '';
}
...
function setCouplingMode(mode){
 if(!current||couplingMutationDepth>0)return;withCouplingMutation(()=>{read({skipAutomatic:true});const next=String(mode||'flexible');current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};current.coupling_mode=next;const context=window.KeySuiteCoupling?.contextFromItems?.(current.items||[])||{};let item=(current.items||[]).find(entry=>entry.section==='coupling'&&entry.couplingData)||null;
 if(!item){const built=window.KeySuiteCoupling?.buildAssemblyItem?.(next,context);item=built?normalizeItem({...built,id:uid(),section:'coupling'},'pumpset'):createCouplingPlaceholder(current,context);current.items.push(item)}
 item.couplingData={...(item.couplingData||{}),selectionMode:next,type:next==='tyre'?'tyre':next==='pin_bush'?'pin_bush':item.couplingData?.type||'pin_bush',autoSelected:true,manualModel:false,_assemblySyncKey:'',_assemblyErrorKey:''};const result=window.KeySuiteCoupling?.configureAssemblyItem?.(item,{selectionMode:next},context);const signature=couplingSyncSignature(current,item,context);
 if(result?.error){item.couplingData={...(item.couplingData||{}),selectionMode:next,type:next==='tyre'?'tyre':item.couplingData?.type||'pin_bush',autoSelected:true,manualModel:false,validationReasons:result.reasons||[result.error],_assemblyErrorKey:signature,_assemblySyncKey:''};item.description=window.KeySuiteCoupling?.assemblyDescription?.(next)||item.description;current.coupling_error=result.error}else{item.couplingData={...(item.couplingData||{}),_assemblySyncKey:couplingSyncSignature(current,item,context),_assemblyErrorKey:''};current.coupling_error=(result?.config?.validationReasons||[]).join('; ')}
 finishCouplingMutation()})
}
function updateCouplingItemFromRow(row,changedClass=''){
 if(!current||couplingMutationDepth>0)return;withCouplingMutation(()=>{read({skipAutomatic:true});const item=current?.items?.find(entry=>entry.id===row?.dataset.assemblyItem);if(!item?.couplingData)return;
 const context=window.KeySuiteCoupling?.contextFromItems?.(current.items||[])||{},values={selectionMode:row.querySelector('.assembly-coupling-type')?.value||current.coupling_mode||item.couplingData.selectionMode||'flexible'};current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};current.coupling_mode=values.selectionMode;
 ...
 const requestedModel=values.model,requestedArrangement=values.arrangement,result=window.KeySuiteCoupling?.configureAssemblyItem?.(item,values,context);if(result?.error){item.couplingData={...(item.couplingData||{}),selectionMode:values.selectionMode,type:values.selectionMode==='tyre'?'tyre':item.couplingData?.type||'pin_bush',model:requestedModel||item.couplingData?.model||item.model,arrangement:requestedArrangement||item.couplingData?.arrangement||'',validationReasons:result.reasons||[result.error],_assemblyErrorKey:couplingSyncSignature(current,item,context),_assemblySyncKey:''};current.coupling_error=result.error}else{item.couplingData={...(item.couplingData||{}),_assemblySyncKey:couplingSyncSignature(current,item,context),_assemblyErrorKey:''};current.coupling_error=(result?.config?.validationReasons||[]).join('; ')}
 finishCouplingMutation()})
}
function restoreCoupling(){if(!current||couplingMutationDepth>0)return;current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};current.coupling_error='';syncAutomaticComponents(current);finishCouplingMutation()}
...
function renderItems(){
 ...
 box.querySelectorAll('input').forEach(i=>i.oninput=()=>{const row=i.closest('[data-assembly-item]'),item=current?.items?.find(entry=>entry.id===row?.dataset.assemblyItem);if(['pump','motor'].includes(item?.section))current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};if(i.classList.contains('assembly-qty'))current.description_manual=false;read();renderItems();renderQuoteFields();scheduleAutoSave()});
 ...
 box.querySelectorAll('.assembly-coupling-type').forEach(select=>select.onchange=()=>setCouplingMode(select.value));box.querySelectorAll('.assembly-coupling-restore').forEach(button=>button.onclick=restoreCoupling);
 box.querySelectorAll('.assembly-delete').forEach(b=>b.onclick=()=>{read({skipAutomatic:true});const row=b.closest('[data-assembly-item]'),item=current.items.find(x=>x.id===row?.dataset.assemblyItem);if(item){current.description_manual=false;current.description=removeDescriptionBlock(current.description,item.description);if(item.section==='coupling')current.auto_suppressed={...(current.auto_suppressed||{}),coupling:true};if(['pump','motor'].includes(item.section))current.auto_suppressed={...(current.auto_suppressed||{}),coupling:false};if(autoComponent(item,'manifold'))current.auto_suppressed={...(current.auto_suppressed||{}),manifold:true};if(autoComponent(item,'tank'))current.auto_suppressed={...(current.auto_suppressed||{}),tank:true}}current.items=current.items.filter(x=>x.id!==row.dataset.assemblyItem);syncAutomaticComponents(current);rebuildDescription(current);if($('assemblyDescription'))$('assemblyDescription').value=current.description||'';renderDescriptionPreview();localSave();renderItems();renderQuoteFields();scheduleAutoSave(100)});
...
async function addItem(item,explicitRoute){
 ...
 const targetWasListed=drafts.some(x=>x.id===target.id),before=JSON.parse(JSON.stringify(target));if(!targetWasListed)drafts.unshift(target);target.customer_id=customerId;target.quote_session_id=session;if(type==='system'&&route.section==='pumpset')target.auto_suppressed={manifold:false,tank:false,coupling:false};if(type==='pumpset'&&['pump','motor'].includes(route.section))target.auto_suppressed={...(target.auto_suppressed||{}),coupling:false};
