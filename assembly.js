...
function blank(t=type){return {id:uid(),assembly_type:t,model_item:'',description:'',description_manual:false,name:t==='system'?'New System':'New Pumpset',customer_id:quoteCustomerId(),quote_session_id:quoteSessionId(),status:'draft',quote_qty:1,quote_unit_price:null,quote_price_manual:false,display_capacity:false,capacity_value:'',capacity_unit:t==='pumpset'?'m³/h':'L/s',head_value:'',head_unit:'Mtr',coupling_mode:'flexible',coupling_error:'',auto_suppressed:{manifold:false,tank:false},items:[],created_at:new Date().toISOString(),updated_at:new Date().toISOString()}}
...
function couplingOptions(item){
 ...
 return `<div class="assembly-motor-options assembly-coupling-options"><div class="assembly-item-option"><label>Selected Type</label><select class="assembly-coupling-type"><option value="flexible" ${mode==='flexible'?'selected':''}>Flexible</option><option value="pin_bush" ${mode==='pin_bush'?'selected':''}>Pin &amp; Bush</option><option value="tyre" ${mode==='tyre'?'selected':''}>Tyre</option></select></div><div class="assembly-item-option"><label>Model</label><select class="assembly-coupling-model"${modelDisabled}>${couplingModelOptions(type,data.model)}</select></div>${type==='tyre'?`<div class="assembly-item-option"><label>F/H Arrangement</label><select class="assembly-coupling-arrangement"${modelDisabled}>${couplingArrangementOptions(data,context)}</select></div>`:''}</div>${bushSummary}`;
}
