...
function blank(t=type){return {id:uid(),assembly_type:t,model_item:'',description:'',description_manual:false,name:t==='system'?'New System':'New Pumpset',customer_id:quoteCustomerId(),quote_session_id:quoteSessionId(),status:'draft',quote_qty:1,quote_unit_price:null,quote_price_manual:false,display_capacity:false,capacity_value:'',capacity_unit:'m³/hr',head_value:'',head_unit:'Mtr',coupling_mode:'flexible',coupling_error:'',auto_suppressed:{manifold:false,tank:false},items:[],created_at:new Date().toISOString(),updated_at:new Date().toISOString()}}
...
function syncCouplingItems(d=current){
 ...
 if(!item){item=normalizeItem({id:uid(),section:'coupling',model:'',bomDescription:'No suitable Coupling model',description:window.KeySuiteCoupling?.assemblyDescription?.(d.coupling_mode)||'',qty:Math.max(1,Number(context.couplingQty||1)),unitPrice:0,pricingSource:{product_family:'COUPLING',pricing_mode:'assembly'},couplingData:{selectionMode:d.coupling_mode,type:d.coupling_mode==='tyre'?'tyre':'pin_bush',autoSelected:true,validationReasons:['No suitable Coupling model is available.']}},'pumpset');d.items.push(item)}
 ...
 if(result?.error){item.couplingData={...(item.couplingData||{}),selectionMode:d.coupling_mode,type:d.coupling_mode==='tyre'?'tyre':item.couplingData?.type||'pin_bush',validationReasons:result.reasons||[result.error]};d.coupling_error=result.error;return}
 d.coupling_error=(result?.config?.validationReasons||[]).join('; ');if(previous&&previous!==item.description)d.description=replaceDescriptionBlock(d.description,previous,item.description);
...
function setCouplingMode(mode){
 if(!current)return;const next=String(mode||'flexible');current.coupling_mode=next;read();const context=window.KeySuiteCoupling?.contextFromItems?.(current.items||[])||{};let item=(current.items||[]).find(entry=>entry.section==='coupling'&&entry.couplingData)||null;
 ...
 if(!item){current.coupling_error='No suitable Coupling model is available for the selected Pump and Motor.';renderItems();scheduleAutoSave(100);return}
 item.couplingData={...(item.couplingData||{}),selectionMode:next,autoSelected:true,manualModel:false};const result=window.KeySuiteCoupling?.configureAssemblyItem?.(item,{selectionMode:next},context);current.coupling_error=result?.error||'';
...
function updateCouplingItemFromRow(row,changedClass=''){
 ...
 const context=window.KeySuiteCoupling?.contextFromItems?.(current.items||[])||{},values={selectionMode:row.querySelector('.assembly-coupling-type')?.value||current.coupling_mode||item.couplingData.selectionMode||'flexible'};current.coupling_mode=values.selectionMode;
 ...
 const result=window.KeySuiteCoupling?.configureAssemblyItem?.(item,values,context);if(result?.error){current.coupling_error=result.error;renderItems();return}current.coupling_error=(result?.config?.validationReasons||[]).join('; ');
 current.description_manual=false;syncQuoteUnitPrice(current);rebuildDescription(current);current.description=normalizeDescriptionIndentation(current.description);if($('assemblyDescription'))$('assemblyDescription').value=current.description||'';renderDescriptionPreview();localSave();renderItems();renderQuoteFields();scheduleAutoSave(100);
}
...
function renderItems(){
 ...
 box.querySelectorAll('.assembly-coupling-type').forEach(select=>select.onchange=()=>setCouplingMode(select.value));
...
async function addItem(item,explicitRoute){
 ...
 const targetWasListed=drafts.some(x=>x.id===target.id),before=JSON.parse(JSON.stringify(target));if(!targetWasListed)drafts.unshift(target);target.customer_id=customerId;target.quote_session_id=session;if(type==='system'&&route.section==='pumpset')target.auto_suppressed={manifold:false,tank:false};
