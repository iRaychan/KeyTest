/* KeySuite V3.8.3 — forward-compatible Dashboard Duty Finder + KeyCore full-screen hubs */
(function(){
'use strict';
if(window.top!==window.self)return;
if(window.__KEYSUITE_V383_EXTENSION__)return;
window.__KEYSUITE_V383_EXTENSION__=true;

var STYLE_ID='ks-v383-extension-style';
function addStyles(){
...
document.addEventListener('click',function(e){if(!document.body.classList.contains('ks-keycore-active'))return;var b=e.target.closest('button,a,[role="button"],[data-action]');if(!b||b.closest('#ksV383Hub'))return;var txt=(b.textContent||'').replace(/\s+/g,' ').trim();if(/^Products?$/i.test(txt)){e.preventDefault();e.stopImmediatePropagation();openHub('product');return}if(/^Pump\s*Curves?$/i.test(txt)||/^Curves?$/i.test(txt)){e.preventDefault();e.stopImmediatePropagation();openHub('curve')}},true);
...
window.KeySuiteV383={openHub:openHub,runDashboardDuty:runDashboardDuty};
